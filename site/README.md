...
`assets/` віддається з `Cache-Control: immutable, max-age=31536000` (рік).
Тому при заміні іконки, банера чи іншого файлу з цієї теки **міняйте ім'я**
(`Icon-512-v2.png` → `Icon-512-v3.png`) і оновлюйте посилання в
`manifest.json` та `index.html`. Інакше браузери й далі показуватимуть
стару версію — навіть після Ctrl+F5.
